...
int main(void)
{
	...
	//wait till the user button is pressed
	...
	if(SPI_verifyresponse(ackbyte))
	{
		...
		printf("CMD_LED_CONTROL executed \n");
	}
	...
	while( !GPIO_ReadFromInputPin(GPIOA, GPIO_PIN_NO_0));
		...
		SPI_SendData(SPI2,arg,1);
		SPI_SendData(SPI2,message,arg[0]);
		printf("COMMAND_PRINT Executed \n");
		}


	//5. COMMAND_ID_READ <len(2)> <message(len)>
			while( !GPIO_ReadFromInputPin(GPIOA, GPIO_PIN_NO_0));
				delay();
